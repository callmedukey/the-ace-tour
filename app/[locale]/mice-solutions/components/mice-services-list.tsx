...
import { MiceService } from "@/db/schemas";
...
const MiceServicesList = ({
  ...
  const list =
    miceServicesData?.length > 0
      ? miceServicesData.map((item) => ({
          title: locale === "ko" ? item.KOtitle : item.ENGtitle,
          description: locale === "ko" ? item.KOcontent : item.ENGcontent,
        }))
      : [
          {
